...
if(isset($_POST['store'])){
	...
	$temp_filename = file_get_contents($file["tmp_name"]);

	if($name && $il && $tags && $source && $file){
		if(uploads($name,$il,base64_encode($temp_filename),$tags,$source,$username,$rv) == "1"){
			echo "upload success";
		}
...
if ($key == 1){
	...
	<td><span>File name</span></td><td><input type='text' name='name'></td></tr><tr><td><span><input type='file' name="files"></span></td></tr><tr>
		<td>Remark</td><td><input type='text' name='il'></td></tr>
		<tr><td>Tags</td><td><input type='text' name='tags'></td></tr>
		<tr><td>No of Replace variable</td><td><input type='text' name='rv'></td></tr>
		<tr style="display: none;"><td>password</td><td><input type='text' name='password' value=<?php echo $_REQUEST['password'];?>></td></tr>


		<tr style="display:none;"><td>username</td><td><input type='text' name='username' value=<?php echo $_REQUEST['username'];?>></td></tr>
...
<?php
	...
	echo "<script>alert('invalid login');window.open('admin.html','_self')
	</script>";
};
?>